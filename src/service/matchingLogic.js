...
export const findMatchingClaims = async (documentEntities, claimIndexer) => {
    try {
        const allClaims = await claimIndexer.getAllIndexedClaims();
        
        const matchResults = allClaims
            .map(claim => calculateDocumentMatchScore(documentEntities, claim))
